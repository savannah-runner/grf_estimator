...
textData = strrep(textData, 'calf_mass', sprintf('%.8f', 0.66085637));

textData = strrep(textData, 'calf_CoM_x', sprintf('%.8f', 0.00084883));
textData = strrep(textData, 'calf_CoM_y', sprintf('%.8f', 0.0));
textData = strrep(textData, 'calf_CoM_z', sprintf('%.8f', -0.43318161));

textData = strrep(textData, 'calf_inertia_ixx', sprintf('%.8f', 0.01167899));
textData = strrep(textData, 'calf_inertia_ixy', sprintf('%.8f', 0.0));
textData = strrep(textData, 'calf_inertia_ixz', sprintf('%.8f', -0.00014067));
textData = strrep(textData, 'calf_inertia_iyy', sprintf('%.8f', 0.01173520));
textData = strrep(textData, 'calf_inertia_iyz', sprintf('%.8f', 0.0));
textData = strrep(textData, 'calf_inertia_izz', sprintf('%.8f', 0.00046139));
...
textData = strrep(textData, 'foot_joint_z', sprintf('%.8f', -0.3475)); %% Length of Calf: Knee-to-Foot length.
textData = strrep(textData, 'foot_collision_sphere_radius', sprintf('%.8f', 0.039));
